...
- Add List and Phases board modes, native pointer movement and keyboard controls, inline creation and renaming, configuration drawers, and guarded deletion.
- Add typed API operations and database storage for all Boards data and authorization rules.
- Keep each Boards RPC contract with its operation, use Outlaw-owned scoped transactions, and organize domain and UI code by responsibility.
- Isolate transient Boards UI state and pointer movement so drag feedback does not rerender whole task collections.
...
- Uses native Pointer Events plus Mantine controls, drawers, popovers, theme colors, and the existing Phosphor icon family.
- Adds service/API tests and interaction coverage for routing, keyboard behavior, native pointer movement, and responsive scrolling.
- Extends sibling Outlaw with adapter-neutral interactive transactions, a Bun implementation, and migration-safe capability forwarding; Cloudflare D1 retains batch-only scripts.
- Removes Task detail excerpts from List rows and Phase cards so details remain in the notebook surface and List search matches visible titles only.
